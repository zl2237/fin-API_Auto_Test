...
本项目面向物流管理系统，采用分层设计，支持从**新建订单**到**审核生成开票申请**的 16 条链路端到端测试，覆盖订单全生命周期、审批流及费用单生成。

**核心能力：**
- 16 条链路，覆盖从新建到审核生成开票申请完整流程（按需执行，灵活组合）
...
│
├── testcases/                    # 测试用例层
│   └── test_link.py              # 16 条链路测试（pytest 标记控制）
│
├── utils/                        # 工具模块
│   ├── logger.py                 # 日志工具
│   └── file_util.py             # 文件操作工具
...
> 链路按依赖顺序递增：link8 隐含 link7 的全部步骤，link16 隐含 link1~link15 的全部步骤。
