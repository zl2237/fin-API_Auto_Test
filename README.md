...
│   └── invoice.pdf
│
├── testcases/                   # 测试用例层（平铺，8 个 test_*.py）
│   ├── __init__.py
│   ├── conftest.py
...
│   ├── test_billing.py          # 链路 13~14：应收对账
│   ├── test_invoice.py          # 链路 15~17：应收发票（开票批次→审核→上传登记）
│   └── test_writeoff.py         # 链路 18：应收核销
│
├── utils/                       # 工具模块
│   ├── logger.py                # 日志工具
│   └── file_util.py             # 文件操作工具
...
pytest -m link18           # 从链路18开始（包含LK1~LK17所有前置步骤）
```
...
### 财务应收对账 API（`api/receive/billing_api.py`）
