...
本项目面向物流管理系统，采用分层设计，支持从**新建订单**到**付款单核销**的 25 条链路端到端测试，覆盖订单全生命周期、审批流、费用单生成、发票上传与登记、应收核销、应付对账、应付开票申请、应付发票上传与登记、发起付款需求、审核生成付款单、付款单核销。

**核心能力：**
- 25 条链路，覆盖从新建到付款单核销完整流程（按需执行，灵活组合）
...
│   └── pay/                     # 付款域（pay）
│       ├── payable_api.py        # 应付对账批次接口封装（含 LK19/20/21/22/23/24/25 全链路）
│       ├── pay_demand_api.py     # 发起付款需求接口封装（LK23）
│       ├── pay_demand_audit_api.py # 审核生成付款单接口封装（LK24）
│       ├── pay_writeoff_api.py   # 付款单核销接口封装（LK25）
│       └── __init__.py
│
├── config/                       # 配置层
...
│   └── pay/                     # 付款域（应付对账 + 应付开票申请 + 应付发票上传与登记 + 发起付款需求 + 审核生成付款单 + 付款单核销）
...
│   └── pay/                      # 付款域测试
│       ├── __init__.py
│       └── test_pay.py           # 链路 19/20/21/22/23/24/25：应付对账、确认应付对账、应付开票申请、应付发票上传与登记、发起付款需求、审核生成付款单、付款单核销
│
├── utils/                       # 工具模块
│   ├── logger.py                # 日志工具
│   └── file_util.py             # 文件操作工具
...
| link21 | 发起应付开票批次申请 | ... → **确认应付对账** → **发起应付开票批次申请**（financePayList + getOrderInfoByFeeId + batchOrderEdit）         |
...
| link25 | 付款单核销 | ... → **审核生成付款单** → **付款单核销**（formPage + writeoffPayFormList + orderFeePage + writeoffBatch）     |
...
OrderWorkflow.run_until_pay_writeoff()        # 到付款单核销（link25）
```
...
| `data/pay/pay_writeoff.yaml` | 付款单核销配置（formPage 查询条件、writeoffBatch 核销参数等） |
