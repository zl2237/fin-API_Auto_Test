...
本项目面向物流管理系统，采用分层设计，支持从**新建订单**到**应付发票上传与登记**的 22 条链路端到端测试，覆盖订单全生命周期、审批流、费用单生成、发票上传与登记、应收核销、应付对账、应付开票申请、应付发票上传与登记。

**核心能力：**
- 22 条链路，覆盖从新建到应付发票上传与登记完整流程（按需执行，灵活组合）
...
│   └── pay/                     # 付款域（pay）
│       ├── payable_api.py        # 应付对账批次接口封装（含 LK19/20/21/22 全链路）
│       └── __init__.py
│
├── config/                       # 配置层
...
│   └── pay/                      # 付款域测试
│       ├── __init__.py
│       └── test_pay.py           # 链路 19/20/21/22：应付对账、确认应付对账、应付开票申请、应付发票上传与登记
│
├── utils/                       # 工具模块
│   ├── logger.py                # 日志工具
│   └── file_util.py             # 文件操作工具
...
> 链路按依赖顺序递增：link22 隐含 link1~link21 的全部步骤。
