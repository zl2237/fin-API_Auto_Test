...
def login():
    try:
        ...
        current_app.logger.info(f"Login attempt for phone: {phone}")
        ok, error = validate(phone, password)
        if not ok:
            current_app.logger.warning(f"Login failed for phone {phone}: {error}")
            return jsonify({"ok": False, "message": error}), 401
        token = make_token(phone)
        role = get_role(phone)
        user = user_service.get_by_phone(phone)
        current_app.logger.info(f"Login successful for phone: {phone}, role: {role}")
        return jsonify({
            "ok": True,
            "token": token,
        ...
        current_app.logger.exception(f"Error in login: {e}")
        return jsonify({"ok": False, "message": f"登录失败: {str(e)}"}), 500
...
def me():
    try:
        ...
        current_app.logger.exception(f"Error in /api/auth/me: {e}")
        return jsonify({"ok": False, "message": f"获取/更新用户信息失败: {str(e)}"}), 500
...
def check_phone():
    try:
        ...
        current_app.logger.exception(f"Error in check_phone: {e}")
        return jsonify({"ok": False, "message": f"检查手机号失败: {str(e)}"}), 500
