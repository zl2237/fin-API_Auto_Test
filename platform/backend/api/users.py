...
def list_users():
    try:
        ...
        current_app.logger.exception(f"Error in list_users: {e}")
        return jsonify({"ok": False, "message": f"获取用户列表失败: {str(e)}"}), 500
...
def create_user():
    try:
        ...
        current_app.logger.info(f"Creating user: {username}, role: {role}, operator: {operator['username']}")
        try:
        ...
        current_app.logger.exception(f"Error in create_user: {e}")
        return jsonify({"ok": False, "message": f"创建用户失败: {str(e)}"}), 500
...
def update_user(user_id: int):
    try:
        ...
        if phone is not None and user_service.is_phone_taken(str(phone).strip(), exclude_user_id=user_id):
            return jsonify({"ok": False, "message": "手机号已存在"}), 400
        current_app.logger.info(f"Updating user {user_id}, operator: {operator['username']}")
        try:
        ...
        current_app.logger.exception(f"Error in update_user({user_id}): {e}")
        return jsonify({"ok": False, "message": f"更新用户失败: {str(e)}"}), 500
...
def delete_user(username: str):
    try:
        ...
        current_app.logger.info(f"Deleting user {username}, operator: {operator['username']}")
        try:
        ...
        current_app.logger.exception(f"Error in delete_user({username}): {e}")
        return jsonify({"ok": False, "message": f"删除用户失败: {str(e)}"}), 500
